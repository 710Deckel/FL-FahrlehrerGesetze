# 👨‍🏫 FL-FahrlehrerGesetze - Fahrlehrer-Gesetze Editor (Erweitert)

**Fahrlehrer-Ausbildung FL-BE_07/25**  
Justin Lee Probis

---

## 📚 Über dieses Repository

Dieses Repository enthält die Daten für den **Fahrlehrer-Gesetze Editor** - ein professionelles Werkzeug zur systematischen Aufbereitung aller relevanten Fahrlehrer-Gesetze und Verordnungen für die Fahrlehrer-Ausbildung.

Der Editor ermöglicht die strukturierte Erfassung von Gesetzestexten zu Fahrlehrer-Erlaubnis, Ausbildung, Prüfung, Fahrschulbetrieb und deren Synchronisation über GitHub.

---

## 📂 Repository-Struktur

```
FL-FahrlehrerGesetze/
├── README.md       (diese Datei - Projektbeschreibung)
└── data.json       (Fahrlehrer-Gesetze-Daten, automatisch synchronisiert vom Editor)
```

---

## 📋 Enthaltene Gesetze & Verordnungen

Dieses Tool vereint alle wichtigen Fahrlehrer-Gesetze in einem Editor:

- **FahrlG** - Fahrlehrergesetz
- **DV-FahrlG** - Durchführungsverordnung zum Fahrlehrergesetz
- **FahrlAusbV** - Fahrlehrer-Ausbildungsverordnung
- **FahrschAusbO** - Fahrschüler-Ausbildungsordnung
- **FahrlPrüfV** - Fahrlehrerprüfungsverordnung
- **Fahrschullernmittel-RL** - Richtlinie für die Ausstattung von Fahrschulen mit Lehrmitteln
- **Verz-Ausbildungsstätten** - Verzeichnis der amtlich anerkannten Fahrlehrerausbildungsstätten
- **Sonstiges** - Platzhalter für weitere Vorschriften

---

## 🔄 Daten-Synchronisation

Die Datei `data.json` wird **automatisch** vom Fahrlehrer-Gesetze Editor synchronisiert:

- ✅ Jedes Speichern im Editor aktualisiert diese Datei
- ✅ Änderungen werden mit Zeitstempel versioniert
- ✅ Team-Kollaboration möglich (mehrere Nutzer, ein Repository)

**⚠️ WICHTIG:** Die `data.json` sollte **nicht manuell bearbeitet** werden!  
Alle Änderungen bitte nur über den Fahrlehrer-Gesetze Editor vornehmen.

---

## 🛠️ Verwendung

### 1. Repository-Setup (einmalig)
- Repository erstellt: ✅ `710Deckel/FL-FahrlehrerGesetze`
- README.md hochgeladen: ✅

### 2. Editor-Verwendung
- HTML-Datei lokal öffnen (`fahrlehrer-gesetze-editor.html`)
- GitHub Token eingeben (einmalig, gleicher Token wie andere Tools!)
- Gesetz aus Dropdown auswählen (FahrlG, DV-FahrlG, FahrlAusbV, FahrschAusbO, FahrlPrüfV, Fahrschullernmittel-RL, Verz-Ausbildungsstätten, Sonstiges)
- Paragraphen hinzufügen und speichern
- Automatische Synchronisation erfolgt

### 3. Token-Anforderungen
Der verwendete Token benötigt folgende Berechtigungen:
- ✅ `repo` (Full control of private repositories)

**Hinweis:** Du kannst denselben Token für alle Fahrlehrer-Tools verwenden!

---

## 📋 Datenstruktur

Die `data.json` enthält alle Paragraphen aus allen Fahrlehrer-Gesetzen im folgenden Format:

```json
{
  "paragraphen": [
    {
      "id": "timestamp",
      "gesetz": "FahrlG",
      "nummer": "§ 1",
      "titel": "Fahrlehrer-Erlaubnis",
      "gesetzestext": "...",
      "quelle": "https://...",
      "praxisbeispiele": [
        {
          "titel": "Beispiel",
          "klasse": "ALLE",
          "beschreibung": "...",
          "wichtigkeit": "CRITICAL"
        }
      ]
    }
  ]
}
```

---

## 🎯 Features des Editors

- **2-Spalten-Layout:** Gesetzestext | Praxisbeispiele
- **Gesetz-Auswahl:** Dropdown für 8 verschiedene Gesetze/Verordnungen
- **GitHub Auto-Sync:** Automatische Synchronisation
- **Template-System:** Vordefinierte Beispiele für Fahrlehrer-Ausbildung
- **Badge-System:** CRITICAL (rot) | HIGH (orange) | BANAL (grün)
- **PDF-Export:** Professionelle Druckausgabe
- **Import/Export:** JSON-Backup-System
- **Keyboard Shortcuts:** Strg+S zum Speichern
- **Indigo Theme:** Speziell für Ausbildungsthemen

---

## 👨‍🏫 Wichtige Bereiche

Der Editor deckt alle wichtigen Bereiche der Fahrlehrer-Gesetze ab:

**FahrlG (Fahrlehrergesetz):**
- § 1 - Fahrlehrer-Erlaubnis
- § 2 - Voraussetzungen
- § 9 - Ruhen und Erlöschen
- § 10 - Widerruf

**DV-FahrlG (Durchführungsverordnung):**
- § 4 - Fachliche Eignung
- § 5 - Persönliche Eignung
- § 8 - Prüfung

**FahrlAusbV (Fahrlehrer-Ausbildungsverordnung):**
- § 2 - Ausbildungsstätte
- § 3 - Fahrlehrerlehrgang
- § 6 - Fahrlehrerprüfung

**FahrschAusbO (Fahrschüler-Ausbildungsordnung):**
- § 1 - Theoretischer Unterricht
- § 4 - Praktische Ausbildung
- § 5 - Ausbildungsnachweis

**FahrlPrüfV (Fahrlehrerprüfungsverordnung):**
- Prüfungsanforderungen
- Prüfungsabläufe
- Bewertungskriterien

**Fahrschullernmittel-RL (Richtlinie):**
- Ausstattungspflichten für Fahrschulen
- Lehrmittel-Anforderungen
- Unterrichtsmaterialien

**Verz-Ausbildungsstätten:**
- Amtlich anerkannte Ausbildungsstätten
- Voraussetzungen für Anerkennung
- Standorte und Kontaktdaten

---

## 🔗 Weitere Fahrlehrer-Tools

Dieses Repository ist Teil einer systematischen Tool-Suite für die Fahrlehrer-Ausbildung:

- [📘 FL-StVO](https://github.com/710Deckel/stvo-teleprompter) - StVO Teleprompter mit Erläuterungen
- [🚛 FL-FPersV-EG-VO](https://github.com/710Deckel/FL-FPersV-EG-VO) - Fahrpersonalverordnung & EU-Verordnung
- [⚖️ FL-StVG](https://github.com/710Deckel/FL-StVG) - Straßenverkehrsgesetz
- [🔧 FL-StVZO](https://github.com/710Deckel/FL-StVZO) - Straßenverkehrs-Zulassungs-Ordnung
- [📋 FL-FeV](https://github.com/710Deckel/FL-FeV) - Fahrerlaubnis-Verordnung
- [🚗 FL-FZV](https://github.com/710Deckel/FL-FZV) - Fahrzeug-Zulassungsverordnung
- [👨‍🏫 FL-FahrlehrerGesetze](https://github.com/710Deckel/FL-FahrlehrerGesetze) - Fahrlehrer-Gesetze (dieses Repository - ERWEITERT)
- [⚖️ FL-VerkehrsrechtStrafrecht](https://github.com/710Deckel/FL-VerkehrsrechtStrafrecht) - Verkehrsrecht & Strafrecht
- [📝 FL-Pruefungsrichtlinie](https://github.com/710Deckel/FL-Pruefungsrichtlinie) - Prüfungsrichtlinie Kfz
- [📚 FL-SonstigeVerkehrsvorschriften](https://github.com/710Deckel/FL-SonstigeVerkehrsvorschriften) - Sonstige Verkehrsvorschriften

---

## 📝 Lizenz & Verwendung

**Projekt:** Fahrlehrer-Ausbildung FL-BE_07/25  
**Ersteller:** Justin Lee Probis  
**Zweck:** Ausbildung und Podcast "Fahrlehrer Inside"

Dieses Tool und die Daten sind für **Ausbildungszwecke** erstellt.

---

## 📞 Kontakt & Feedback

Bei Fragen, Problemen oder Verbesserungsvorschlägen:
- GitHub Issues in diesem Repository
- Feedback über das Tool (Thumbs Down Button)

---

**Erstellt mit ❤️ für die Fahrlehrer-Ausbildung**

*Letzte Aktualisierung: Dezember 2024*
